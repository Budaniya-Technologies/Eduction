...
const Dashboard = ({
                        ...
                        <ul className="ml-6 mt-2 space-y-1 border-l border-gray-200 pl-4">
                          {item.subItems.map((sub, subIdx) => (
                            <li
                              ...
                              onClick={() => {
                                onSectionClick(sub)
                                setActiveSection(sub) // <--- This is the fix!
                                setSidebarOpen(false)
                              }}                              
                            >
                              {sub}
  ...
  {activeSection === null ? (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {cards.map((card, i) => (
          <div
            key={i}
            className={`bg-white rounded-lg shadow-sm p-6 flex flex-col items-center transform hover:scale-105 transition-transform duration-200 border-t-4 border-${card.color}-500`}
          >
            <p className={`text-4xl font-extrabold text-${card.color}-600 mb-2`}>{card.count}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="lg:col-span-2 bg-white rounded-lg shadow-sm p-6">
          <h4 className="text-xl font-semibold text-gray-800 mb-4">Student Engagement</h4>
          <iframe
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h4 className="text-xl font-semibold text-gray-800 mb-4">Content Upload Distribution</h4>
          <iframe
    ...
    <div className="bg-white rounded-lg shadow-sm p-6 overflow-y-auto min-h-[400px]">
      {mainComponent}
    </div>
  )}
</main>
