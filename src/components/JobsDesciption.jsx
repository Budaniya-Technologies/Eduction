...
const JobsDescription = () => {
                    ...
                    >
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-sm text-gray-600">{job.category?.name || 'Category'}</p>
                                <p className="text-xs text-gray-500">{job.job_type.replace('_', ' ')}</p>
                                <p className="text-xs text-gray-400 mt-1">
                                    {new Date(job.posted_at).toLocaleDateString('en-IN')} · Exp: {job.experience_min}–{job.experience_max} yrs
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-white shadow-md rounded-xl p-6 max-w-4xl mx-auto">
                    ...
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600">{selectedJob.subcategory?.name}</p>
                            <div className="text-xs text-gray-500 mt-1">
                                {selectedJob.job_type.replace('_', ' ')} · {selectedJob.salary_min}K–{selectedJob.salary_max}K
                            </div>
                        </div>
                    </div>

                    <div className="flex flex-wrap gap-4 text-sm text-gray-500 mb-6">
                        <span>Posted: {new Date(selectedJob.posted_at).toLocaleDateString('en-IN')}</span>
                        <span>Deadline: {new Date(selectedJob.application_deadline).toLocaleDateString('en-IN')}</span>
                        <span>
                            Experience: {selectedJob.experience_min}–{selectedJob.experience_max} yrs
                        </span>
                    </div>
    ...
    return (
        <div className="flex flex-col md:flex-row h-screen bg-[#f9f9f9]">
            {isMobile ? (showDetailMobile ? renderJobDetail() : renderJobList()) : <>
                {renderJobList()}
                {renderJobDetail()}
            </>}
        </div>
