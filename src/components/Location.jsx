...
const Location = ({ children }) => {
  ...
  useEffect(() => {
    // Check if location was already granted during this session
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          sessionStorage.setItem("locationAccessGranted", "true"); // Save status
          setLocationGranted(true);
        } catch (error) {
  ...
  // Show popup only if not yet granted
  if (!locationGranted) {
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-pink-200 via-yellow-100 to-blue-200 z-50 flex flex-col items-center justify-center text-center px-6">
        <div className="bg-white shadow-2xl rounded-3xl p-10 max-w-md w-full border-4 border-dashed border-blue-300 animate-fadeIn">
          ...
          <p className="text-lg text-blue-800 mb-4 font-semibold">
            We need your location to help you explore cool things nearby!
          </p>
          ...
          {loading ? (
            <p className="text-purple-500 animate-pulse text-md font-semibold mb-4">
